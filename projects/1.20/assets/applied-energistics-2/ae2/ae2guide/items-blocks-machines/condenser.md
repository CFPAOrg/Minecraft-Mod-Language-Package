...
*   在物质球模式下，物质聚合器会用输入物品制造<ItemLink id="matter_ball" />。此模式需要在聚合器顶部槽位装有存储元件。制造物质球需消耗256个物品或256桶流体。
    each, so a <ItemLink id="cell_component_1k" /> (which provides 8192 bits of capacity) is more than enough.
*   在奇点模式下，物质聚合器会用输入物品制造[奇点](singularities.md)。此模式需要在聚合器顶部槽位装有存储元件。制造奇点需消耗256000个物品或256000桶流体。
