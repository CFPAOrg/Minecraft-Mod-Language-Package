...
与普通参数共同使用时，贪婪运算符必须位于参数列表中的最后一位。

;;;;;

宏是转译器独有的特性，戏法师的法术无法使用LISP的宏，也根本无法将LISP宏转换成法术片段。


向卷轴存储法术时，会将整段代码转换成法术，且会以附加数据的形式保留宏。
...
而当宏存储在已佩戴的任意戒指（譬如**宏戒指**）中时，转译器同样能识别到它们，并依此还原出宏调用！
