...
过滤器**按频道设置，不按节点**。过滤器槽仅属于**单个频道**——[标题](header.md)中选择的频道。切换到其他频道后，过滤器槽会改为包含该频道的过滤器。也即，频道的6个过滤器槽相互独立，单个频道可接受6项过滤器配置。
...
升级**按节点设置，不按频道**。与过滤器正好相反。节点的**所有6个频道**共享升级。放入一个钻石级升级，则所有频道都将获得更大的单次数量和更短的延迟。无需（也无法）单独为频道安装升级。
