...
- **类型**：频道正在传输的资源，**物品**、**流体**、**能量**。各个类型使用相应的颜色。
- **节点计数**：该网络中已启用节点的个数。

各行左侧还有一个匹配频道类型的**带色标记**——绿色为物品，蓝色为流体，以此类推。它可以用来快捷确定频道类型，无需查看右侧文本。

聚合**按频道索引**进行，不按节点。若有10个节点同时在CH0上进行物品传输，那么列表内只会有一行`CH0 物品 10 个节点`，而不会分成10行。对其中一行按下**主要交互键（默认：左键）**可查看详细图表。

## 吞吐量图表

![I/O监视器吞吐量图表，CH0的实时数据峰值约为200K/s](../computer/images/io-monitor-graph.png)

对频道行按下主要交互键可打开其图表。图表会显示该频道的**实时传输吞吐量时间线**：
