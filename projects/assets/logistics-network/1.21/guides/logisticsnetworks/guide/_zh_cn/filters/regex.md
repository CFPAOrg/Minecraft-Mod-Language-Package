...
使用正则表达式，根据**显示名称**或**提示文本**匹配物品。很强大，也很容易误用——如需在高流通量频道中使用，务必阅读下方的警告。

## 匹配对象

所有所选文本（名称、工具提示、两者）匹配所设正则表达式的物品。匹配根据游戏内的显示效果决定，与注册ID无关。
...
- **范围**：匹配哪种文本：
  - **名称**（默认）：仅物品的显示名称。
  - **工具提示**：仅物品的提示文本（说明/lore、NBT提示等），不匹配名称。
  - **两者**：名称与所有提示文本。
- **白名单/黑名单**切换按钮。
...
- 匹配所有在提示文本中提及“精准采集”的附魔书 → 范围为工具提示，模式为`精准采集`。
- 匹配所有名称以“铁”起始的物品 → 范围为名称，模式为`^铁`。
- 阻止所有在提示文本中提及“受损”进入存储器具 → 范围为工具提示，模式为`受损`，黑名单。
...
| `精准采集`*（范围为工具提示）* | 包含“精准采集”的提示文本             | 带有精准采集魔咒的附魔书/装备      |
| `等级 \d+`*（范围为工具提示）* | “等级”后跟数字的提示文本             | 带等级的物品、经验书               |
...
- **[regex101.com](https://regex101.com/)**：实时正则表达式测试器，带有解释面板。将侧栏中的基准（flavor）设为**Java 8**，以便和本过滤器中的行为保持一致。
