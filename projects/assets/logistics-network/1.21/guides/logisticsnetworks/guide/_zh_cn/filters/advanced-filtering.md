...
小型、中型、大型过滤器分别有9、18、27个过滤条目槽。最简单的使用方法是直接放入物品，不过其实每个槽位都有隐藏的**详情**页面，内有条目独有的选项，可让过滤器的匹配能力大大超出精确物品匹配。
...
也可以两者都不使用——留空该输入框，只使用NBT规则，或只使用槽位设置。未设置物品/标签、但设置了NBT规则的条目会匹配所有符合NBT条件的物品。
...
每次传输移动多少物品，仅作用于该条目，覆盖频道设置。此设置**不会**与频道的单次数量设置**求和**——它只会为该条目的传输数量设置上限。
...
点击**配置**可打开此条目的NBT规则页面。NBT规则可在物品/标签匹配之上进一步匹配物品的数据组件（魔咒、损坏值、自定义名称、堆叠数量、自定义NBT标签等）。
...
- **原始SNBT**（**Raw SNBT**）：可供粘贴原始SNBT段的文本框。条目会匹配包含所有指定NBT的物品。仅用作高级功能——适合给无法在表格中简洁表示的匹配模式使用。
...
- **仅不可堆叠物品**：添加规则`堆叠数量`（`Stack Size`）`=` `1`。适用于路由单个受损的工具，不会发送完好物品的堆叠。
...
有效槽位索引范围为**0到53**（标准Minecraft容器的最大值就在53上下）。留空可让条目访问方块公开的所有槽位。
...
- **详情页面的清空**：清空条目的所有设置：物品/标签、单次数量、库存量、槽位、所有NBT规则。
- **NBT子页面的清空**：仅清空NBT规则；物品/标签、单次数量、库存量、槽位均不变。
