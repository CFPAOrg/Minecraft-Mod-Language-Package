...
namespace Packer
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using var stream = File.Create("./Minecraft-Mod-Language-Package"); // 生成空 zip 文档
            using var archive = new ZipArchive(stream);
            InitializeArchive(archive, config);
            var existingModids = new HashSet<string>();
            var assetsToBePacked = new DirectoryInfo($"./projects/{config.Version}")
            ...
            foreach (var asset in assetsToBePacked)
            {
                ...
                if (existingModids.Contains(id))
                {
                    ...
                    foreach(var file in asset.assetPath.EnumerateFiles())
                    {
                        var destinationPath = $"assets/{id}/{file.FullName[asset.prefixLength..]}";
