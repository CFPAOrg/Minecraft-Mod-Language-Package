...
with open('config.yml', 'r') as f:
    config = yaml.load(f)
    MOD_PAGE = config['mod_page']
    VERSION = config['version']
    BLACK_LIST = config['mod_blacklist']
    WHITE_LIST = config['mod_whitelist']
...
def func_page_download(i):
    ...
    for url_name in url_name_list:
        ...
        # Project ID 获取
        mod_url = 'https://minecraft.curseforge.com/projects/' + url_name
        project_id = get_info(mod_url, r'<div class="info-data">(.*?)</div>')
...
with open('logs/mods/' + csv_time + '.csv', 'w') as c:
